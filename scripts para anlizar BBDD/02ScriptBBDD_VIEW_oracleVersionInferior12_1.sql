...
/****************************** INICIO CREACIÓN TABLA ALL_VIEWS VERSIÓN ORACLE INFERIOR A 12.1 ******************************************    	 
   1. Realizar un repace de DBA_XXXX al esquema que vayamos a analizar: por ejempo, DBA_APET, DBA_AYMV
   2. Descomentar y una vez creada se puede quitar/comentar este fragmento.
 */
--DROP TABLE TEMP_ALL_VIEWS;

/*  
  CREATE TABLE TEMP_ALL_VIEWS (
        OWNER VARCHAR2(128),
        ...
        EDITIONING_VIEW CHAR(1),
        READ_ONLY CHAR(1)
    );

   
BEGIN
    FOR r IN (SELECT OWNER, VIEW_NAME, TEXT_LENGTH, TEXT, VIEW_TYPE_OWNER, VIEW_TYPE, SUPERVIEW_NAME, EDITIONING_VIEW, READ_ONLY
              FROM ALL_VIEWS
              WHERE OWNER = 'DBA_XXXX')  -- Indicar nombre esquema a analizar
              LOOP                  
        INSERT INTO TEMP_ALL_VIEWS (OWNER, VIEW_NAME, TEXT_LENGTH, TEXT, VIEW_TYPE_OWNER, VIEW_TYPE, SUPERVIEW_NAME, EDITIONING_VIEW, READ_ONLY)
        VALUES (r.OWNER, r.VIEW_NAME, r.TEXT_LENGTH, r.TEXT, r.VIEW_TYPE_OWNER, r.VIEW_TYPE, r.SUPERVIEW_NAME, r.EDITIONING_VIEW, r.READ_ONLY);                  
    END LOOP;    
    COMMIT;
END;
/
*/
-- probamos que se creó y contiene datos 
--SELECT * FROM TEMP_ALL_VIEWS
--WHERE UPPER(TEXT) LIKE '%SUCA%';

/******************************FIN CREACIÓN TABLA ALL_VIEWS VERSIÓN ORACLE INFERIOR A 12.1 ******************************************/*/

/**
 * Códigos de las aplicaciones en las que buscar ocurrencias en la BBDD
 * Quitar/Comentar la aplicación que estéis analizando
 ...
 */
, eventos AS (
    SELECT evento.owner || '_'|| POAPS.codigo "ID_EVENTO", POAPS.*, evento.* 
    FROM TEMP_ALL_VIEWS evento
    JOIN POAPS  ON  UPPER(TEXT) LIKE '% ' || POAPS.codigo || '!_%' ESCAPE '!'  -- Patrón. Ejemplo: '% SOIC_'  
